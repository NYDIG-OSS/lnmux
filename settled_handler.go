...
type SettledHandler struct {
	...
	waiters     map[lntypes.Hash][]chan types.InvoiceStatus
...
func NewSettledHandler(cfg *SettledHandlerConfig) *SettledHandler {
	return &SettledHandler{
		...
		lnds:      cfg.Lnds,
		waiters:   make(map[lntypes.Hash][]chan types.InvoiceStatus),
	}
}

func (p *SettledHandler) InvoiceSettled(hash lntypes.Hash, status types.InvoiceStatus) {
	p.waitersLock.Lock()
...
func (p *SettledHandler) WaitForInvoiceFinalEvent(ctx context.Context,
	hash lntypes.Hash) (types.InvoiceStatus, error) {

	logger := p.logger.With("hash", hash)

	waitChan := make(chan types.InvoiceStatus, 1)
